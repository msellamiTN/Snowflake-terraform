...
| `terraform import` échoue : `object does not exist` | Nom d'objet incorrect ou pas encore créé | Vérifier que l'objet existe dans Snowflake : `snow sql -c training -q "SHOW DATABASES LIKE 'BROWNFIELD_DEV'"` |
...
| `terraform validate` échoue : `Reference to undeclared variable` | `var.learner_prefix` n'existe pas dans le starter | Utiliser le nom littéral de la database (par exemple `BROWNFIELD_DEV`) au lieu d'une variable |
