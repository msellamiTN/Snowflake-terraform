# 🧪 Lab M2 — State Terraform collaboratif sur Azure Blob Storage

| Élément | Valeur |
|---|---|
| **Durée** | 120 min |
| **Pistes** | `[CORE]` `[COLLAB]` `[INCIDENT]` `[PRODUCTION]` |
...
Vous êtes Data Platform Engineer. Le state créé dans M1 se trouve encore sur votre poste. Vous devez le migrer vers Azure Blob Storage, isoler les sandboxes des apprenants, permettre à deux développeurs de collaborer sans écriture concurrente, diagnostiquer les incidents courants et préparer une procédure de recovery contrôlée.
...
- un mini-projet `terraform_data` démontre la collaboration et le Blob Lease sans modifier Snowflake ;
...
- DEV, UAT et PROD possèdent des clés de state distinctes.

## 🗺️ Vue globale des scénarios

```mermaid
flowchart LR
    C[INCIDENT C<br/>risques du state local] --> A[CORE A<br/>migration APP]
    A --> F[CORE F<br/>preuves distantes]
    F --> B[COLLAB B<br/>deux développeurs]
    B --> D[COLLAB D<br/>Blob Lease]
    D --> E[INCIDENT E<br/>auth et backend]
    E --> G[PRODUCTION G<br/>isolation et composition]
    G --> H[PRODUCTION H<br/>recovery guidé]
```

**Lecture :** le parcours commence par les limites du state local, construit le backend individuel, démontre la collaboration sur une stack séparée, puis traite les incidents et les pratiques de production.
...
- ✅ migrer un state local sans recréer les ressources ;
...
| Stack collaborative | `TEAM01` | `training/TEAM01/collab/terraform.tfstate` | Scénarios `[COLLAB]` |
| Environnements séparés | `APP01` | `training/APP01/<env>/terraform.tfstate` | Cible `[PRODUCTION]` |
...
flowchart TD
    ...
    TEAM --> ENV[Une clé par environnement]
    APP --> ENV
```
...
Ces protections sont nécessaires pour le scénario de recovery.
...
## 🔒 Scénario D `[COLLAB]` — Observer le Blob Lease

```mermaid
sequenceDiagram
    participant A as Dev A
    participant TF1 as Terraform A
    participant L as Lease du blob TEAM01
    participant TF2 as Terraform B
    participant B as Dev B
    A->>TF1: apply -replace
    TF1->>L: Acquire lease
    L-->>TF1: Lease accordé
    B->>TF2: plan -lock-timeout=0s
    TF2->>L: Acquire lease
    L-->>TF2: Lease déjà détenu
    TF2-->>B: Error acquiring state lock
    A->>TF1: Ctrl+C
    TF1->>L: Release lease
    B->>TF2: plan
    L-->>TF2: Lease accordé
...
### D.1 — Maintenir le verrou dans le terminal A

```powershell
terraform apply -replace=terraform_data.shared_marker
```

Laissez Terraform attendre sur `Enter a value:`. Ne saisissez pas `yes`.

### D.2 — Tester depuis le terminal B
...
Dans le terminal A, utilisez `Ctrl+C`. Dans le terminal B :
...
## 🏭 Scénario G `[PRODUCTION]` — Isolation et composition

**Durée : 10 min avant recovery.**

### G.1 — Concevoir les clés par environnement
...
```mermaid
flowchart TB
    APP[Propriétaire APP01] --> DEV[DEV<br/>training/APP01/dev/terraform.tfstate]
    APP --> UAT[UAT<br/>training/APP01/uat/terraform.tfstate]
    APP --> PROD[PROD<br/>training/APP01/prod/terraform.tfstate]
    DEV -. aucune écriture croisée .- UAT
    UAT -. aucune écriture croisée .- PROD
```

M2 ne déploie pas UAT ou PROD. Cette isolation sera mise en œuvre dans M8.

✅ **Checkpoint G1** : l’apprenant peut expliquer pourquoi une clé distincte limite le blast radius et évite la confusion de workspace.

### G.2 — Lire un output avec `terraform_remote_state`
...
✅ **Checkpoint G2** : le lecteur affiche le nom de la database RAW de M1.
...
- [ ] APP et TEAM utilisent des clés différentes ;
...
1. pourquoi `training/APP01/dev/terraform.tfstate` et `training/APP01/prod/terraform.tfstate` doivent être séparés ;
...
| Critère | Points |
|---|---:|
| Choix APP/TEAM correctement justifié | 3 |
| Preuves migration et lock | 3 |
| Diagnostic auth/RBAC | 2 |
| Garde-fous recovery | 2 |
...
1. Dans quel cas une clé TEAM est-elle préférable à une clé APP ?
