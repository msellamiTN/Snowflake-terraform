...
> **Lab CORE (70 min) + stretch optionnel (50 min) — scénarios A, F, D (CORE) puis E, C, B, G, H (stretch).** Procéder dans l’ordre des checkpoints. Capturer uniquement des preuves non sensibles. Ne jamais afficher ni partager une clé de stockage, un jeton, un mot de passe, le contenu de `.env` ou un state complet.
...
## � Checkpoint F — Preuves distantes
...
## 🅳 Checkpoint D — Verrou Azure Blob (Blob Lease)
...
- **Prévention** — Annoncer les opérations d’écriture et utiliser exactement la clé CORE `training/APP01/dev/terraform.tfstate` pour l’exercice de lock.
...
- **Symptôme** — `terraform plan` dans le dossier B propose `1 to add` malgré un code identique au dossier A.
- **Cause** — Les deux dossiers ont des states locaux indépendants ; B ne connaît pas la ressource créée par A.
- **Diagnostic** — Comparer `terraform state list` dans A et B. Le code est identique mais les states divergent.
- **Correction** — C'est le résultat pédagogique attendu. La conclusion est que le state local crée une divergence.
- **Prévention** — Utiliser un backend distant partagé (scénario A) pour éviter la divergence.

## �️ Checkpoint B — Collaboration TEAM01

### Le second développeur voit `1 to add` au lieu de `No changes`

- **Symptôme** — Le développeur B obtient un plan avec création au lieu de `No changes`.
- **Cause** — B a utilisé `terraform init -migrate-state` avec un state local existant, au lieu de `terraform init` simple.
- **Diagnostic** — Vérifier la commande utilisée par B. Le backend TEAM01 est déjà initialisé par A.
- **Correction** — B doit utiliser `terraform init` (sans `-migrate-state`) pour rejoindre la stack existante.
- **Prévention** — Seul le premier développeur initialise la stack. Les suivants font `terraform init` simple.

## 🅶 Checkpoint G — `terraform_remote_state`
